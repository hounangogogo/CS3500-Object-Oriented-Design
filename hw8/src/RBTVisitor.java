...
/**
 * A visitor for MyMap that was created with MyMap.empty(c) It is the responsibility of the user of
 * this interface to guarantee that it is only used for Red-Black Trees
 *
...
public interface RBTVisitor<K, V, R> {
   ...
   */
  public R visitEmpty(Comparator<? super K> comp, boolean red);
   ...
   * @return some value of the type R
   */
  public R visitNode(Comparator<? super K> comp,
                     boolean red, K k, V v,
                     MyMap<K, V> left,
                     MyMap<K, V> right);
}
