...
public class Node<K, V> extends MyTree<K, V> {
  ...
  public MyTree<K, V> include(K key, V value) {
    if (key.equals(k0)) {
      ...
      return new Node<>(k0, v0, left.set(key, value), right, comp, red).balance();
    } else {
      return new Node<>(k0, v0, left, right.set(key, value), comp, red).balance();
    }
  }
  ...
  public MyTree<K, V> accept(MyMapVisitor<K, V> visitor) {
    MyTree<K, V> le = left.accept(visitor);
    MyTree<K, V> ri = right.accept(visitor);
    return new Node<>(k0, visitor.visit(k0, v0),
            le, ri, comp, true);
  }
  ...
  public <R> R acceptRBT(RBTVisitor<K, V, R> visitor) {
    if (isRed()) {
      return visitor.visitNode(comp, true, k0, v0, left, right);
    }
    else {
      return visitor.visitNode(comp, false, k0, v0, left, right);
    }
  }
